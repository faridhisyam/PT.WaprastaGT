...
// Tambahkan method end() untuk menghentikan USB
bool ESPUSB::end() {
  if (!_started) {
    return false; // USB belum dimulai
  }
  
  // Hentikan stack TinyUSB, fungsi ini harus tersedia pada library TinyUSB
  // Jika fungsi tinyusb_deinit() tidak tersedia, Anda bisa menggunakan tusb_stop() (pastikan sudah dideklarasikan)
  tusb_stop();
