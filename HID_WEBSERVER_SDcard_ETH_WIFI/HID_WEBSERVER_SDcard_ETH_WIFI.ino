...
const int myFunction = 6;
const int myBACA = 7;       // Pin tombol

int WIFI = 0, ETHERNET = 0, USBorKEY = 0;  
...
void setup() {
  ...
  pinMode(myFunction, INPUT_PULLDOWN);
  pinMode(myBACA, INPUT_PULLUP);
  ...
  USBorKEY = digitalRead(myFunction);
}
...
void loop() { 
  if (flagSetup == 1) {  
    ...
    if(WIFI == 1 && ETHERNET == 0)
      { 
        if(USBorKEY == 1) {
          stopUSBMSC();
          Tulis("Flashdisk Mode", "Tengah");
          delay(1000);
          startUSBMSC();   
        }
        else if(USBorKEY == 0){
          Tulis("Keyboard Mode", "Tengah");
          delay(1000);    
          initKeyboard();
        }
  ...
  else 
  {
    ...
    // if(USBorKEY == 0 && WIFI == 0 && ETHERNET == 0 || USBorKEY == 0 && WIFI == 1 && ETHERNET == 0 || USBorKEY == 0 && WIFI == 0 && ETHERNET == 1 ) { 
    if(USBorKEY == 0 && WIFI == 0 && ETHERNET == 1 ) { 
